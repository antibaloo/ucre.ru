...
$dom = new domDocument("1.0", "utf-8"); // Создаём XML-документ версии 1.0 с кодировкой utf-8
  ...
  if(CModule::IncludeModule('iblock')) {
      ...
      $dom->save("/home/bitrix/ucre.ru/orenburg_ucre_domclick.xml"); // Сохраняем полученный XML-документ в файл
      echo "Выгружено: ".$num;
      $time = microtime(true) - $start;
      printf(' Скрипт выполнялся %.4F сек.', $time);
      CEventLog::Add(array(
         "SEVERITY" => "SECURITY",
         "AUDIT_TYPE_ID" => "DMCLK_EXPORT",
         "MODULE_ID" => "main",
         "ITEM_ID" => 'Каталог недвижимости',
         "DESCRIPTION" => "Выгрузка агентом объектов недвижимости в формате Яндекс.Недвижимость для ДомКлик, выгружено ".$num." объектов за ".$time." секунд",
      ));
    }
echo "Выгрузка агентом объектов недвижимости в формате Яндекс.Недвижимость для ДомКлик, выгружено ".$num." объектов за ".$time." секунд";
